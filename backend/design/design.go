...
var _ = dsl.Service("service", func() {
	...
	dsl.Method("addTodoTask", func() {
		...
		dsl.Payload(func() {
			...
			dsl.Attribute("description", dsl.String, "Todoタスクの説明")
			dsl.Attribute("asignee", dsl.String, "タスクを割り当てられた人の名前")
			dsl.Attribute("startDate", dsl.String, "Todoタスクの開始予定日", func() {
				dsl.Format(dsl.FormatDate)
			})
			dsl.Attribute("endDate", dsl.String, "Todoタスクの終了予定日", func() {
				dsl.Format(dsl.FormatDate)
			})
			dsl.Required("title", "startDate", "endDate")
		})
		dsl.HTTP(func() {
			dsl.POST("/add")
